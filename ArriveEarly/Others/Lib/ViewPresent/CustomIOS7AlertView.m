...
- (void)showCorner:(CGFloat )corner
{
    ...
    dialogView.layer.cornerRadius = corner;
...
- (void)addButtonsToView: (UIView *)container
{
    ...
    for (int i=0; i<[buttonTitles count]; i++) {
        ...
        //由于当设置标题多个的时候 没有一条分割线问题，自己加上一个
        if (i>0) {
            UIImageView *imgviewLine = [[UIImageView alloc] initWithFrame:CGRectMake(CGRectGetWidth(closeButton.frame)*i, CGRectGetMinY(closeButton.frame)-1, 1, CGRectGetHeight(closeButton.frame))];
            imgviewLine.backgroundColor = [UIColor whiteColor];
            [container addSubview:imgviewLine];
        }
